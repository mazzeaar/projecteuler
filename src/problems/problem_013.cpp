...
size_t solve_problem_13()
{
    // cache would be much better if we rotated num...
    // we also probably dont need all of the numbers, but hey it works
    std::vector<std::vector<size_t>> num = {
    ...
    size_t carry = 0;
    std::vector<size_t> result_row; // this number is in reverse
    for ( int col = W - 1; col >= 0; --col ) {
        for ( int row = 0; row < H; ++row ) {
            carry += num[row][col];
        }

        result_row.push_back(carry % 10);
        carry /= 10;
    }
    ...
    for ( size_t i = 0; i < 10; ++i ) {
        const size_t idx = result_row.size() - 1 - i;
        result = result * 10 + result_row[idx];
    }

    ASSERT_EQUAL(result, 5537376230);
    return result;
}

REGISTER_PROBLEM(solve_problem_13, 13)
