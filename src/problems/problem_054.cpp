...
struct Hand {
   ...
   private:
     ...
     */
    template <int N> inline score_t tuple_score() {
        score_t score = 0;
        for (int i = rank.size() - 1; i >= 0; --i) {
            if (rank[i] == N) {
                score |= (0b1 << i);
                rank[i] = 0;  // we set the counted cards to zero to avoid recounting them for N=1
                              // (highest card)
            }
        }
    ...
    template <typename... Args> void generate_score(int hand_score, Args... args) {
