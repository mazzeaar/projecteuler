...
void print(double runtime, long int solution = 0)
{
    if ( solution == 0 )
    {
        std::cout << "\nSolution to problem 11 is not implemented yet!\n" << std::endl;
        return;
    }
...
int main()
{
    ...
    int maxh = horizontal(grid);
    int maxv = vertical(grid);
    int maxdlr = diagonal_l_r(grid);
    int maxdrl = diagonal_r_l(grid);

    solution = max(max(max(maxh, maxv), maxdlr), maxdrl);
